...
#' \enumerate{
#'   \item The Davies test p-value is below \strong{p.values.threshold}.
...
#' half-width \strong{slope_check_window_size}.  Only candidates that also pass
#' both slope p-value thresholds are recorded.
#'
#' After the sliding-window loop, each accepted changepoint is refined to the
#' nearest local peak in \strong{y} via \code{\link{get_local_maximum}}.
#'
...
#'   to retain a validated changepoint.  Candidates with
#'   \code{y[tau_hat] <= min_signal} are removed after the Davies filter.
#'   Default \code{2.5}.
#'
#' @details
...
#' If either Davies p-value is below \strong{p.value.threshold}, the candidate
...
#' \code{y[tau_hat] <= min_signal} is removed, as such positions are below
