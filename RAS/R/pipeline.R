...
#' \code{num_rep} independent 50/50 train/hold-out splits.
#'
#' @param geno Matrix of genotype dosages (\eqn{n} samples \eqn{\times}
#'   \eqn{N} variants). Rows must be aligned with \code{phenotype} and
#'   \code{covariates}.
...
#' @param covariates Data frame with \eqn{n} rows aligned with \code{geno}.
#'   Must contain all columns named in \code{covariate_cols}; additional
#'   columns (e.g., an ID column) are silently ignored.
#' @param covariate_cols Character vector of column names in \code{covariates}
...
#'
#' @details
#' For each of the \code{num_rep} repetitions the function performs three
#' steps:
#'
#' \enumerate{
...
#' divided by \code{num_rep} to form the final profile.
#'
...
#'     \eqn{-\log_{10}(p)}-value profile across all \code{num_rep}
#'     repetitions.}
#' }
...
#'         (re-mapped to genomic coordinates via \code{this.start} and
#'         \code{this.skip}).}
