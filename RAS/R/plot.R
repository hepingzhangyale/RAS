...
#' @param detection.result List. Output from \code{\link{second_scanning}},
...
#' \itemize{
#'   \item Red (\code{#d73027}): \eqn{\ge} \code{p.threshold}
#'   \item Orange (\code{#fc8d59}): \eqn{\ge} \code{p.threshold / 2}
#'   \item Yellow (\code{#fee090}): \eqn{\ge 2.5}
#'   \item Blue (\code{#91bfdb}): below \code{2.5}
#' }
...
#' the total x-range.  When \code{y_cap} is set, positions above the cap are
...
#' it exceeds \code{p.threshold}).  Vertical dashed segments connect markers
#' to the baseline.
#'
...
plot_ras_scan <- function(x, y, detection.result, this_chrom, save.directory,
  ...
  plot(x, y_display, type = "n",
       main  = paste0("Chr ", this_chrom,
                      "  RAS Scan  (changepoint p-threshold = 1e-",
                      round(-log10(1e-8)), ")"),
       xlab  = "SNP Index",
  ...
  abline(h = 2.5,         col = "grey55", lty = 3, lwd = 0.8)
  ...
  abline(h = 2.5,      col = "grey60", lty = 3, lwd = 0.8)
...
#' the significance thresholds (2.5, \code{p.threshold / 2},
#' \code{p.threshold}).  Left and right slope lines are plotted in dark green
...
plot_ras_zoom_regions <- function(x, y, detection.result,
  ...
  for (k in seq_along(tau_hats)) {
    ...
    abline(h = 2.5,         col = "grey55", lty = 3, lwd = 0.7)
    ...
    for (thr in c(2.5, p.threshold / 2, p.threshold)) {
      over <- yz >= thr
      if (!any(over)) next
