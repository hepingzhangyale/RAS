...
# --- changePoint_detection_window_scanning ---

test_that("changePoint_detection_window_scanning returns list with required elements", {
  set.seed(42)
  x      <- 1:200
  y      <- make_peak_series(200, peak_pos = 100)
  result <- changePoint_detection_window_scanning(x, y, window_size = 100, skip = 10,
...
test_that("changePoint_detection_window_scanning output vectors are consistent length", {
  set.seed(42)
  x      <- 1:200
  y      <- make_peak_series(200, peak_pos = 100)
  result <- changePoint_detection_window_scanning(x, y, window_size = 100, skip = 10,
...
test_that("second_scanning returns list with required elements", {
  set.seed(42)
  n <- 300
  x <- 1:n
  ...
  result <- second_scanning(mock_first, x = x, y = y,
